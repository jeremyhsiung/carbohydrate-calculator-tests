...
class CarbohydrateCalculatorTest {
    ...
    void tc005_metricMifflinCalculation() {
        ...
        enter(By.id("ckg"), "80");
        new Select(driver.findElement(By.id("cactivity"))).selectByVisibleText("Sedentary: little or no exercise");
    ...
    void tc020_usKatchReturnsResults() {
        ...
        assertEquals("standard", driver.findElement(By.id("ctype")).getDomProperty("value"), "The submitted calculation should use US units");
        ...
        String[] goals = {"Weight Maintenance", "Lose 1 lb/week", "Lose 2 lb/week", "Gain 1 lb/week", "Gain 2 lb/week"};
        ...
        List<WebElement> headerCells = rows.get(0).findElements(By.cssSelector("td"));
        ...
        for (int goal = 0; goal < goals.length; goal++) {
            ...
            assertEquals(goals[goal], cells.get(0).getText().replace('\u00a0', ' '));
            // Check that the cell shows a number followed by "Calories".
            assertTrue(cells.get(1).getText().matches("[0-9,]+\\s+Calories"), goals[goal] + ": numeric Calories and unit");
            ...
            for (int column = 2; column < cells.size(); column++) {
                ...
                assertTrue(grams.getText().replace('\u00a0', ' ').matches("[0-9,]+\\s+grams"), goals[goal] + ": numeric carbohydrate target and grams unit");
    ...
    private void assertMetricCalculations(WebElement result, double maintenanceCalories) {
        ...
        String[] goals = {"Weight Maintenance", "Lose 0.5 kg/week", "Lose 1 kg/week", "Gain 0.5 kg/week", "Gain 1 kg/week"};
        ...
        // Assumption: the site appears to use 3.75 Calories per gram
        // (2136 * 40% / 3.75 rounds to 228 g).
        //
        // The assignment does not define this conversion or the
        // 500/1000 calorie changes for each goal.
        //
        // These checks follow the observed results.
        // Those rules still need confirmation.
        ...
        for (int goal = 0; goal < goals.length; goal++) {
            ...
            assertEquals(Math.round(expectedCalories), number(cells.get(1)), goals[goal] + ": Calories");
            for (int column = 0; column < percentages.length; column++) {
                ...
                assertEquals(expectedGrams, number(carbohydrateCell), goals[goal] + ": carbohydrate grams at " + headers[column + 2]);
            }
        }
    }
    ...
    private WebElement calculate() {
        ...
        // Calculate loads a new page.
        // Wait for the old form to disappear so we do not read old results.
    ...
    private long number(WebElement cell) {
        // Read one whole-number value, such as
        // "2,136 Calories" or "228 grams".
        //
        // This removes everything except digits,
        // so do not use it for decimals, negatives or multiple values.
